...
describe('ArtifactTagHelper', () => {
  ...
  test('resolveTags existing and new', async () => {
  ...
  test('getTags invalid item', async () => {
    db.query.mockResolvedValue({
      items: [
        {
          pk,
    ...
    await expect(helper.getTags({ userId })).rejects.toThrow('Unknown error');
  });
});
